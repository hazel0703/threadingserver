...
void requestHandle(int fd, long arrival, long dispatch) {
	...
	if (strcasecmp(method, "GET")) { /*The strcasecmp() function shall compare, while ignoring differences in case, the string pointed to by s1 to the string pointed to by s2. The strncasecmp() function shall compare, while ignoring differences in case, not more than n bytes from the string pointed to by s1 to the string pointed to by s2.
...
*/
        requestError(fd, method, "Error 501", "The server has not GET method programmed", "Method unavailable at 1DT032 Server");
        return;
        }
